...
void main () {
  ...
  if (heat > 0.0) {
    fireColor = mix(vec3(0.0), vec3(1.0, 0.0, 0.0), clamp((heat - 0.15) * 2.0, 0.0, 1.0)); // Black to red
    fireColor = mix(fireColor, vec3(1.0, 0.5, 0.0), clamp((heat - 0.5) * 2.0, 0.0, 1.0)); // Red to orange
    fireColor = mix(fireColor, vec3(1.0, 1.0, 0.0), clamp((heat - 0.65) * 4.0, 0.0, 1.0)); // Orange to yellow
    fireColor = mix(fireColor, vec3(1.0, 1.0, 1.0), clamp((heat - 0.8) * 10.0, 0.0, 1.0)); // Yellow to white
  }
